...
test('Convert ANSI sequences to regular strings.', t => {
	...
	const result = ansiEncode.encode(ansiEscapedStr)
...
test('Double quotes do not get escaped.', t => {
	...
	const keep = ['"']
	const opts = {keep}
	const result = ansiEncode.encode(ansiEscapedStr, opts)
	t.is(typeof result, 'string')
...
test('Can strip ANSI escape sequences from ansi-encoded strings.', t => {
	const ansiEscapedStr = chalk.green('Test')
	const encoded = ansiEncode.encode(ansiEscapedStr)
	const result = ansiEncode.stripEncoded(encoded)
	t.is(typeof result, 'string')
	t.is(result, 'Test')
})
...
test('Can strip ANSI escape sequences from ansi-escaped strings.', t => {
	const ansiEscapedStr = chalk.green('Test')
	const result = ansiEncode.strip(ansiEscapedStr)
	t.is(typeof result, 'string')
	t.is(result, 'Test')
})
...
test('Multi-line JSON highlighting with fn strings.', t => {
	...
	const encoded = ansiEncode.encode(highlighted, opts)
